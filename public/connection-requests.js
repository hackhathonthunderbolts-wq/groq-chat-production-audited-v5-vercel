/* Add-on: incoming connection request approval. Kept separate from the React app. */
(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .cra-card{background:rgba(255,255,255,.98);border:1px solid rgba(0,0,0,.1);border-radius:18px;box-shadow:0 18px 55px rgba(0,0,0,.18);padding:18px;color:#171717;backdrop-filter:blur(14px);animation:cra-in .22s ease-out}
  ...
  const hide = () => { const root = document.getElementById(ROOT_ID); if (root) { root.innerHTML = ''; root.dataset.requestId = ''; } };
  ...
  async function loadIncoming() {
    ...
    try {
      ...
      const incoming = Array.isArray(rows) ? rows.find(row => row.status === 'pending' && row.addressee_id === state.me) : null;
      if (!incoming) { hide(); return; }
      ...
      root.innerHTML = `
        <div class="cra-card" role="dialog" aria-live="polite" aria-label="Connection request">
          <div class="cra-head"><div class="cra-icon">↗</div><div><div class="cra-title">New connection request</div><div class="cra-sub">${escapeHtml(incoming.name || 'Another user')}</div></div></div>
          <div class="cra-text">This user sent you a connection request. Accept it to enable messaging, or reject it.</div>
          <div class="cra-actions"><button class="cra-btn cra-accept" data-action="accept">Accept</button><button class="cra-btn cra-reject" data-action="reject">Reject</button></div>
        ...
        button.addEventListener('click', async () => {
          ...
          try {
            await request(`/api/connections/${encodeURIComponent(incoming.id)}`, {
              ...
              body: JSON.stringify({ status: action === 'accept' ? 'accepted' : 'declined', userId: state.me })
            });
            hide();
            window.dispatchEvent(new CustomEvent('connection-request-updated'));
          } catch (error) {
    ...
    } catch { /* Database/API may be disabled; do not disturb the existing app. */ }
  }
